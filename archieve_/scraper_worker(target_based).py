...
# Shared cap across ALL workers combined. Once this many threads have been
# successfully scraped in total (by any worker), every worker stops pulling
# new work. Change this number to whatever sample size you want.
TARGET_SCRAPED = 15000
...
def run_worker():
    ...
    with sync_playwright() as p:
        ...
        print(f"Progress   : {int(redis_db.get(SCRAPED_COUNTER) or 0)}/{TARGET_SCRAPED} scraped so far (all workers combined)")
        ...
        try:
            while True:
                # Stop pulling new work once the shared target is reached,
                # no matter which worker got there first.
                if int(redis_db.get(SCRAPED_COUNTER) or 0) >= TARGET_SCRAPED:
                    print(f"\n[TARGET REACHED] {TARGET_SCRAPED} threads scraped — stopping")
                    break
